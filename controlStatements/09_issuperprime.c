// Lists all the prime numbers up to a user-given number N.
// Checks if the number N is a prime number.
// If it is a prime, number, checks if it is also a super-prime number (*),
// by checking its order in the prime number list.

// Copyright (C) 2025 O. Ayhan

// (*) Super-prime numbers, also known as higher-order primes
// or prime-indexed primes (PIPs), are the subsequence of prime numbers
// that occupy prime-numbered positions within the sequence of
// all prime numbers. In other words, if prime numbers are matched
// with ordinal numbers, starting with prime number 2 matched with ordinal number 1,
// then the primes matched with prime ordinal numbers are the super-primes.
// (Source: Wikipedia)
